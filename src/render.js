...
export function details(g,mochiLanding=''){return `<section class="details" hidden><div class="detail-play"><a class="play" href="${g.url}" target="_blank" rel="noopener noreferrer" aria-label="${g.name}を遊ぶ(別タブ)"><span class="play-label"><span aria-hidden="true">▶</span> PLAY</span><img class="play-qr" src="/assets/play-${g.id}-qr.png" alt="${g.name}を開くQRコード" width="116" height="116"></a><p class="detail-note">ゲームを別のタブで開きます。</p></div><p class="eyebrow">MICO MICO NEKOCHAN GAMES / ${g.id.toUpperCase()}</p><h2>${g.name}</h2><p class="lead">${g.lead}</p><img class="detail-cat" src="/assets/${g.cat}" alt=""><p>${g.description}</p><p>${g.extra}</p><div class="detail-facts">${g.facts.map(x=>`<span>${x}</span>`).join('')}</div>${g.id==='mochi'?mochiLanding:''}</section>`;}
